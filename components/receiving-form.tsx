...
const GOOGLE_DRIVE_FOLDER_ID = "1ImiQ8YeVrjB1OrvuC4UhuPXRYanz1FJb";
const PUBLIC_SHEET_ID_MASTER = "1FQwLNqZMJHttaOQnI6Xna5nghHOgQXyWOUfGLAfsfko";
...
const ReceivingForm: React.FC<ReceivingFormProps> = ({ currentUser }) => {
  ...
  const [dropdownOptions, setDropdownOptions] =
    useState<ReceivingDropdownOptions>({ vendorNames: [], modes: [] });
  ...
  const fetchDropdownOptions = async () => {
    ...
    try {
      const response = await fetch(
        `${appScriptUrl}?sheet=${PUBLIC_SHEET_MASTER_NAME}&action=fetch`
      );
      if (!response.ok)
      ...
      if (!jsonData.success || !jsonData.data)
        throw new Error("Master data is not in the expected format.");

      const vendorNamesSet = new Set<string>();
      const modesSet = new Set<string>();
      const dataRows = jsonData.data.slice(1);

      dataRows.forEach((row: any) => {
        if (row[VENDOR_COLUMN_INDEX_IN_MASTER_SHEET] != null)
          vendorNamesSet.add(
            String(row[VENDOR_COLUMN_INDEX_IN_MASTER_SHEET]).trim()
          );
        if (row[MODE_COLUMN_INDEX_IN_MASTER_SHEET] != null)
          modesSet.add(String(row[MODE_COLUMN_INDEX_IN_MASTER_SHEET]).trim());
      });
      ...
      toast({
        title: "Error",
        description: "Could not fetch dropdown options.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleAddNewVendor = async () => {
    ...
    try {
      const url = `https://docs.google.com/spreadsheets/d/${PUBLIC_SHEET_ID_MASTER}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        PUBLIC_SHEET_MASTER_NAME
      )}`;
      ...
      for (let i = 1; i < dataRows.length; i++) {
        if (
          dataRows[i].c?.[VENDOR_COLUMN_INDEX_IN_MASTER_SHEET]?.v != null &&
          String(
            dataRows[i].c[VENDOR_COLUMN_INDEX_IN_MASTER_SHEET].v
          ).trim() !== ""
        ) {
      ...
      let existingRowData: string[] = Array(
        VENDOR_COLUMN_INDEX_IN_MASTER_SHEET + 1
      ).fill("");
      if (targetRowIndex < dataRows.length && dataRows[targetRowIndex]?.c) {
        for (let i = 0; i < dataRows[targetRowIndex].c.length; i++) {
          existingRowData[i] =
            dataRows[targetRowIndex].c[i]?.v != null
              ? String(dataRows[targetRowIndex].c[i].v)
              : "";
        }
      }
      existingRowData[VENDOR_COLUMN_INDEX_IN_MASTER_SHEET] =
        newVendorName.trim();
      ...
      if (result.success) {
        toast({
          title: "Success",
          description: `Vendor "${newVendorName.trim()}" added successfully.`,
        });
        setNewVendorName("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        imageLink = await new Promise<string>((resolve, reject) => {
          reader.onload = async () => {
            ...
            if (uploadResult.success) resolve(uploadResult.fileUrl);
              ...
              reject(new Error(uploadResult.error || "Image upload failed."));
          };
      ...
      const rowDataArray = [
        ...
        formData.date,
        formData.vendorName,
        Number.parseFloat(String(formData.invoiceAmt)) || 0,
      ...
      const submitBody = new URLSearchParams({
        action: "insert",
        sheetName: RECEIVING_DATA_SHEET_NAME,
      ...
      if (submitResult.success) {
        toast({
          title: "Entry Added!",
          description: "Your receiving entry has been recorded.",
        });
        setFormData({
          date: new Date().toISOString().split("T")[0],
          vendorName: "",
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-10">
        ...
        <p className="ml-3 text-slate-600">Loading Form...</p>
      </div>
    );
  }
  if (error) {
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <SelectContent>
                {dropdownOptions.vendorNames.map((v) => (
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onChange={(e) =>
              setFormData((p) => ({
                ...p,
                invoiceAmt: parseFloat(e.target.value) || 0,
              }))
            }
            name="invoiceAmt"
            required
...
const ReceivingRecords: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        const parsedRecords: ReceivingRecord[] = json.table.rows
  ...
  if (error) {
    return (
      <div className="p-6 text-center text-red-700 bg-red-100 rounded-lg">
        <p className="font-bold">Failed to load records</p>
        <p className="text-sm mt-1">{error}</p>
      </div>
    );
  }
        ...
        <TableBody>
          {records.length > 0 ? (
            records.map((rec) => (
              <TableRow
                key={rec.id}
                className="hover:bg-purple-50/60 border-b last:border-b-0 border-slate-100"
              >
                <TableCell className="text-slate-600">
                  {formatDisplayTimestamp(rec.timestamp)}
                </TableCell>
                <TableCell className="text-slate-600">
                  {formatEntryDate(rec.date)}
                </TableCell>
